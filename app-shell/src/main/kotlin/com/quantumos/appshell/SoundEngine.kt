 ...
 *
 * **Moved from `:app` to `:app-shell` in Phase 19.** It lived in `com.quantumos.shell.ui`, and `:app`
 * depends on every docked module and never the reverse — so no docked module could reach it and
 * QUARK's avatar was silent despite her three chirps having been in this bank since M6. The class is
 * unchanged apart from its package; it now sits beside the other things every module shares
 * (PhosphorHueRuntime, SettingsStore, the App Shell chrome), and :app constructs it exactly as
 * before. Every docked module gains sound, not just the avatar.
 */
class SoundEngine {
    ...
    // `internal`, not private: the synthesis is pure Kotlin with no Android types in it, so it can
    // be unit-tested on the JVM. `blast()` is the only part that needs a device.
    internal fun synth(token: String): ShortArray? {
