...
*The single source of truth for the QuantumOS project. Version 0.28*

> **Note (v0.28):** **M6 — splash, sound, polish — confirmed on the Fold 6, alongside the Deployment
> Region patch.** Real fonts (Chakra Petch body-wide, Monoton reserved for the single wordmark-stamp
> moment), the real boot ceremony, a synthesized sound pass, and real AGSL CRT shaders are all in
> place. **One flagged, non-blocking observation:** the Director notes the real shader reads a bit
> more subtle than the earlier demos — kept as-is for now, with intensity tuning earmarked as a
> later adjustment rather than a blocker. The Deployment Region (Japan/Hong Kong) and Boot Pace
> toggles both ship with real persistence, closing the gap flagged when the patch was first written.
> **Next: M7 — ship + field-test — the last milestone before Checkpoint β.** This one is shaped
> differently from the others: its core artifact is a **signing key**, the one truly irreversible
> object in the project so far (code can always be fixed; a lost or leaked signing key cannot), so it
> gets the same explicit-ceremony treatment M1's rollback and M4's permission walkthrough got. A
> dedicated **M7 Task Brief** is banked. Decision **70** added.
>
...
milestone sequence.
70. **[v0.28] M6 confirmed on hardware, alongside the Deployment Region patch.** Real Chakra Petch
typography replaces the Monospace placeholder used since M0; Monoton is confirmed reserved for the
single boot-wordmark moment only. The real boot ceremony, a synthesized sound pass, and real AGSL
CRT shaders are all in place and confirmed working — the shader specifically judged on actual
hardware for the first time, rather than argued about in chat or seen through a software emulator.
**One flagged, non-blocking note:** the real shader reads more subtle than earlier demos; kept as-is
for now, intensity tuning noted as a future adjustment rather than required rework. The Deployment
Region (Japan default / Hong Kong) and Boot Pace (Deliberate default) toggles both ship on STATUS
with real cross-restart persistence. **Next: M7 — ship + field-test, the final milestone before
Checkpoint β.** Its central artifact — a release signing key — is treated with the same explicit
care as M1's rollback rehearsal and M4's permission walkthrough, since it is the one genuinely
irreversible object the project has produced: code can be revised, but a lost or leaked signing key
cannot be recovered or swapped without breaking future updates. A dedicated **M7 Task Brief** is banked.
...
3. ▶️ **Launcher App** *(Trident · Tree 1.5)* — QUARK home as an installable launcher + companion apps. Buildable/testable **on the Z Fold 6 now**; its **build spec (v1.1), persona pack (v1.0), and scripted-line library (v1.1) are all banked [v0.18]** — the M5 scripted brain has its content. → working APK. — **CHECKPOINT β after this**
   ...
   - **3.6a ✅ *(confirmed on hardware [v0.28])* Deployment Region switch** — Japan (default) / Hong Kong, real persistence across restarts. QUARK's acknowledgement lines confirmed on-device, pending final fold into the Scripted-Line Library as canon.
   - **3.7 ✅ *(confirmed on hardware [v0.28])* M6 — Splash, sound, and polish** — real Chakra Petch typography, real boot ceremony, synthesized sound pass, real AGSL CRT shaders (judged on hardware for the first time). Shader intensity flagged as subtle vs. earlier demos — non-blocking, tuning noted for later.
   - **3.8 ▶️ *(next, final milestone before Checkpoint β)* M7 — Ship + field-test** — generate + secure the signing key, build a real signed release APK, sideload, full on-device regression pass across every milestone. Dedicated **M7 Task Brief [v0.28]** banked.
...
*End of Build Bible v0.28 — ask Clara to update and re-version whenever a decision changes.*
