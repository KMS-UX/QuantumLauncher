...
*The single source of truth for the QuantumOS project. Version 0.22*
...
3. ▶️ **Launcher App** *(Trident · Tree 1.5)* — QUARK home as an installable launcher + companion apps. Buildable/testable **on the Z Fold 6 now**; its **build spec (v1.1), persona pack (v1.0), and scripted-line library (v1.1) are all banked [v0.18]** — the M5 scripted brain has its content. → working APK. — **CHECKPOINT β after this**
   - **3.0 ▶️ *(optional, no-hardware)* Pre-M0 Cloud Spike [v0.22]** — from a browser (Android Studio Cloud), build the project skeleton + M0 design-system foundation + the pure-logic modules into a real **debug APK in a cloud emulator**, pushed to the GitHub repo so the ROG inherits a compiling project. Shader fidelity + launcher behavior deferred to hardware. Runbook banked. *(Currently available to run on the trip.)*
...
*End of Build Bible v0.22 — ask Clara to update and re-version whenever a decision changes.*
